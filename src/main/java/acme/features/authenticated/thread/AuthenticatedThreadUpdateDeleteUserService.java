...
@Service
public class AuthenticatedThreadUpdateDeleteUserService implements AbstractUpdateService<Authenticated, Thread> {
	...
	public void bind(Request<Thread> request, Thread entity, Errors errors) {
		...
		int idUsuario = request.getModel().getInteger("usertodelete");
		Collection<Authenticated> authenticateds = this.repository.findAuthenticated(entity.getId());
		authenticateds.remove(this.repository.findUser(idUsuario));
